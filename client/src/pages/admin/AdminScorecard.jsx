...
const AdminScorecard = () => {
  ...
  // --- RESULT BUILDER STATE (NEW) ---
  const [toss, setToss] = useState({ winner: "us", decision: "bat" });
  const [outcome, setOutcome] = useState({ winner: "us", type: "runs", margin: "" }); // 'us', 'them', 'draw', 'tie'
  ...
  useEffect(() => {
    const fetchInfo = async () => {
      try {
        ...
        // Pre-fill Result (Try to parse the string if possible, otherwise default)
  ...
  // --- RENDERERS (BattingRow & BattingTable code remains same as previous version) ---
  ...
  const BattingTable = ({ isUs }) => (
    <div className="bg-white rounded-lg shadow overflow-hidden mb-6">
        ...
        <div className="p-2">{isUs ? squad.map((p, index) => (<div key={p.user_id} draggable onDragStart={() => (dragItem.current = index)} onDragEnter={() => (dragOverItem.current = index)} onDragEnd={handleSort} onDragOver={(e) => e.preventDefault()} className="transition-transform duration-200"><BattingRow isUs={true} p={p} index={index} /></div>)) : oppBatting.map((row, i) => <BattingRow key={i} isUs={false} p={row} index={i} />)}</div>
    </div>
  );
  ...
  const BowlingTable = ({ isUs }) => (
    <div className="bg-white rounded-lg shadow overflow-hidden mb-6">
        ...
        <div className="p-2">{isUs ? squad.map(p => { const s = ourStats[p.user_id] || {}; if(!s.played) return null; return (<div key={p.user_id} className="grid grid-cols-12 gap-1 items-center p-1 rounded hover:bg-gray-50 mb-1 border-b"><div className="col-span-4 text-sm font-semibold">{p.full_name}</div><input type="number" className="col-span-2 p-1 border rounded text-center" value={s.overs} onChange={e=>updateOurStat(p.user_id, 'overs', e.target.value)} /><input type="number" className="col-span-2 p-1 border rounded text-center" value={s.maidens} onChange={e=>updateOurStat(p.user_id, 'maidens', e.target.value)} /><input type="number" className="col-span-2 p-1 border rounded text-center" value={s.runs_given} onChange={e=>updateOurStat(p.user_id, 'runs_given', e.target.value)} /><input type="number" className="col-span-2 p-1 border rounded text-center font-bold text-blue-600" value={s.wickets} onChange={e=>updateOurStat(p.user_id, 'wickets', e.target.value)} /></div>)}) : oppBowling.map((row, i) => (<div key={i} className="grid grid-cols-12 gap-1 items-center p-1 rounded hover:bg-gray-50 mb-1 border-b"><div className="col-span-4"><input type="text" placeholder={`Bowler ${i+1}`} className="w-full p-1 text-sm border rounded" value={row.player_name} onChange={e=>updateOppStat('bowl', i, 'player_name', e.target.value)} /></div><input type="number" className="col-span-2 p-1 border rounded text-center" value={row.overs} onChange={e=>updateOppStat('bowl', i, 'overs', e.target.value)} /><input type="number" className="col-span-2 p-1 border rounded text-center" value={row.maidens} onChange={e=>updateOppStat('bowl', i, 'maidens', e.target.value)} /><input type="number" className="col-span-2 p-1 border rounded text-center" value={row.runs_given} onChange={e=>updateOppStat('bowl', i, 'runs_given', e.target.value)} /><input type="number" className="col-span-2 p-1 border rounded text-center font-bold text-blue-600" value={row.wickets} onChange={e=>updateOppStat('bowl', i, 'wickets', e.target.value)} /></div>))}</div>
    </div>
  );
