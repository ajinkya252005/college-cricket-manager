...
const AdminPlayers = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const alumniRes = await fetch("http://localhost:5000/api/players/alumni");
  ...
  const handleApprove = async (id) => {
    await fetch(`http://localhost:5000/api/players/approve/${id}`, { method: "PUT" });
    toast.success("Player Approved!"); fetchData();
  };

  const handleReject = async (id) => {
    if(window.confirm("Reject request?")) {
        await fetch(`http://localhost:5000/api/players/reject/${id}`, { method: "DELETE" });
  ...
  const handleArchive = async (id) => {
    if(window.confirm("Move to Alumni?")) {
        await fetch(`http://localhost:5000/api/players/archive/${id}`, { method: "PUT" });
  ...
  const submitEdit = async (e) => {
      e.preventDefault();
      try {
          const res = await fetch(`http://localhost:5000/api/players/update/${editPlayer.user_id}`, {
              method: "PUT",
  ...
  const submitCreate = async (e) => {
      e.preventDefault();
      try {
          const res = await fetch("http://localhost:5000/api/players/add", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify(formData)
          });
